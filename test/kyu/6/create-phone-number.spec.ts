...
describe('create-phone-number tests', () => {
  ...
  it('Should pass random tests', () => {
    ...
    for (let i = 0; i < 100; i++) {
      const a: number[] = Array.from({ length: 10 }, () => Math.floor(Math.random() * 10)),
        exp: string = sol(a)
      assert.equal(createPhoneNumber(a), exp, `createPhoneNumber(${JSON.stringify(a)})`)
    }
  })
})
